...
import org.apache.hadoop.mapreduce.{Job => NewAPIHadoopJob, TaskID, SparkHadoopMapReduceUtil}
...
case class OrcTableScan(
  ...
  override def execute(): RDD[Row] = {
    ...
    val job = new NewAPIHadoopJob(sc.hadoopConfiguration)
    ...
    val rowRdd: RDD[Row] = rdd.mapPartitions { iter =>
      fillObject(iter, attributes.zipWithIndex, mutableRow)
    }
    rowRdd
  }
}
