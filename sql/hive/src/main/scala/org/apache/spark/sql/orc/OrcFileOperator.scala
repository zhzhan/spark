...
private[orc] object OrcFileOperator extends Logging{
  ...
  def getFileReader(path: String): Reader = {
    val job = new Job()
    val jobConf = job.getConfiguration.asInstanceOf[JobConf]
    val fs = FileSystem.getLocal(jobConf)
    val orcFiles = listOrcFiles(path, jobConf)
    OrcFile.createReader(fs, orcFiles(0))
  }

  def getSchema(path: String): Seq[Attribute] = {
    val reader = getFileReader(path)
  ...
  def getObjectInspector(path: String): StructObjectInspector = {
    val reader = getFileReader(path)
    val readerInspector: StructObjectInspector = reader.getObjectInspector
      .asInstanceOf[StructObjectInspector]
    readerInspector
