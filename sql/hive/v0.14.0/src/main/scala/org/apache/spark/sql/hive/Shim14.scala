...
private[hive] object HiveShim {
  ...
  def getExternalTmpPath(context: Context, path: Path) = {
    context.getExternalTmpPath(path)
  }
  ...
  def loadPartition(db: Hive, loadPath: Path, tableName: String,
    ...
    db.loadPartition(loadPath, tableName, partSpec, replace, holdDDLTime,
      inheritTableSpecs, isSkewedStoreAsSubdir, isSrcLocal, isAcid)
  ...
  def loadTable(db: Hive, loadPath: Path, tableName: String, replace: Boolean,
    ...
    isAcid: Boolean) = {
    db.loadTable(loadPath, tableName, replace, holdDDLTime, isSrcLocal,
