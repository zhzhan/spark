...
private[hive] object HiveShim {
  ...
  def getArgumentInspectors(functionClassName: String, children: Seq[Expression],
                            func: DataType => ObjectInspector) = {
    if( functionClassName == "org.apache.hadoop.hive.ql.udf.generic.GenericUDFRound" &&
      children.size == 2) {
      Seq(func(children(0).dataType),
        PrimitiveObjectInspectorFactory.getPrimitiveWritableConstantObjectInspector(
          TypeInfoFactory.intTypeInfo, new IntWritable(children(1).eval().asInstanceOf[Int])))
    } else {
      children.map(_.dataType).map(func)
    }
  }
